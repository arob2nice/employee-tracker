...
async function mainMenu() {
    ...
    switch (action) {
        ...
        case 'Add a role':
            ...
            const deptChoices = departmentsList.map((dept) => ({
                name: dept.name,
                value: dept.id,
            }));
        ...
        case 'Add an employee':
            ...
            const managerChoices = employeeList.map((emp) => ({
        ...
        case 'Update an employee role':
            ...
            const employeeChoices = employeesUpdateList.map((emp) => ({
                name: `${emp.first_name} ${emp.last_name}`,
                value: emp.id,
            }));
            const rolesUpdateList = await queryHandler.getAllRoles();
            const roleUpdateChoices = rolesUpdateList.map((role) => ({
                name: role.title,
                value: role.id,
            }));
